...
class GinexDataset:
    def __init__(
        self,
        path="../dataset/ogbn-papers100M-ginex",
        split_idx_path=None,
        score_path=None,
    ):
    ...
    def get_adj_mat(self):
        indptr = np.fromfile(self.indptr_path, dtype=self.conf["indptr_dtype"]).reshape(
            tuple(self.conf["indptr_shape"])
        )
        indices = np.memmap(
            self.indices_path,
            mode="r",
            shape=tuple(self.conf["indices_shape"]),
            dtype=self.conf["indices_dtype"],
        )
        indptr = torch.from_numpy(indptr)
        indices = torch.from_numpy(indices)
        return indptr, indices
    ...
    def get_labels(self):
        labels = torch.from_numpy(
            np.fromfile(
                self.labels_path, dtype=self.conf["labels_dtype"], count=self.num_nodes
            ).reshape(tuple([self.conf["labels_shape"][0]]))
        )
        return labels
