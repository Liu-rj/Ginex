...
class FeatureCache:
        ...
        for t in range(num_threads):
            loader.append(
                threading.Thread(
                    ...
                    args=(
                        n_id_list,
                        list(range(t, len(n_id_list), num_threads)),
                        self.exp_name,
                        self.sb,
                    ),
                )
    ...
    def pass_3(self, iterptr, iters, initial_cache_indices):
        ...
        for t in range(num_threads):
            ...
            loader.append(
                threading.Thread(
                    target=load_into_queue,
                    args=(
                        q[t],
                        list(range(t, self.effective_sb_size, num_threads)),
                        self.exp_name,
                        self.sb,
                    ),
                    daemon=True,
        ...
        for i in range(self.effective_sb_size):
            ...
            # Map table update
            map_table[n_id_cuda] = torch.arange(
                n_id_cuda.numel(), dtype=torch.int32, device="cuda"
            )
            ...
            cache_table[
                candidates[next_access_iters == (threshold + 1)][:num_remains]
            ] |= 4
            del candidates
            ...
            save_p = threading.Thread(
                target=save,
                args=(i, in_indices, in_positions, out_indices, self.exp_name, self.sb),
            )
            save_p.start()
...
class NeighborCache:
    ...
    def init_by_score(self, score):
        ...
        address_table[sorted_indices[:num_entries]] = torch.cat(
            [torch.zeros(1).long(), cumulative_size[: num_entries - 1]]
        )
        ...
        fill_neighbor_cache(
            cache, self.indptr, self.indices, cached_idx, address_table, num_entries
        )
