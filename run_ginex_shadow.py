...
from lib.shadow_sampler import ShaDowKHopSampler
...
def execute(i, cache, pbar, total_loss, total_correct, last, mode='train'):
    ...
    for idx in range(num_iter):
        ...
        if idx != num_iter-1:
            ...
            # Gather
            gather_loader = threading.Thread(target=gather, args=(gather_q, n_id, cache, root_n_id), daemon=True)
            gather_loader.start()
