...
dataset_path = os.path.join("/nvme1n1/ginex_dataset", args.dataset + "-ginex")
...
if args.exp_name is None:
    ...
    args.exp_name = (
        f"{args.dataset}-{args.batch_size}-{args.sizes}-{args.feature_cache_size}"
    )
os.makedirs(os.path.join("/nvme1n1/ginex_dataset/trace", args.exp_name), exist_ok=True)
...
def delete_trace(i):
    n_id_filelist = glob.glob(
        "/nvme1n1/ginex_dataset/" + args.exp_name + "/sb_" + str(i - 1) + "_ids_*"
    )
    adjs_filelist = glob.glob(
        "/nvme1n1/ginex_dataset/" + args.exp_name + "/sb_" + str(i - 1) + "_adjs_*"
    )
    cache_filelist = glob.glob(
        "/nvme1n1/ginex_dataset/" + args.exp_name + "/sb_" + str(i - 1) + "_update_*"
    )
...
def train(epoch):
    ...
    for i in range(num_sb + 1):
        ...
        tic = time.time()
        cache, initial_cache_indices, decompose_time = changeset_compute(
            i, last=(i == num_sb), mode="train"
        )
        torch.cuda.synchronize()
        info_recoder[0] += time.time() - tic
        for j in range(2):
            sample_decompose[j + 2] += decompose_time[j]
...
if __name__ == "__main__":
    ...
    with open("logs/end_to_end_single_thread_decompose.csv", "a") as f:
        ...
        for epoch_info in epoch_info_recoder:
            log_end_to_end.append(round(np.mean(epoch_info[1:]), 2))
    ...
    with open("logs/train_single_thread_decompose.csv", "a") as f:
        ...
        for epoch_info in epoch_train_decompose:
            log_train.append(round(np.mean(epoch_info[1:]), 2))
