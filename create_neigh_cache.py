...
def save_neighbor_cache():
    ...
    dataset = GinexDataset(
        path=dataset_path, split_idx_path=split_idx_path, score_path=score_path
    )
    score = dataset.get_score()
    rowptr, col = dataset.get_adj_mat()
    num_nodes = dataset.num_nodes
    neighbor_cache = NeighborCache(
        args.neigh_cache_size, score, rowptr, dataset.indices_path, num_nodes
    )
    del score
    print("Done!")
